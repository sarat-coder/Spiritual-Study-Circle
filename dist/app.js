(() => {
  ...
  const sourceLink = value => {
    if (/^assets\/(?:bhagavatha-vahini|ramakatha-rasavahini-part-[12]|upanishad-vahini|sutra-vahini|sathya-sai-vahini|sandeha-nivarini)\.pdf(?:#page=[1-9]\d{0,2})?$/.test(value)) return escape(value);
  ...
  function render() {
    ...
    const chapterMode=b.organization==='chapters', unit=chapterMode?'chapter':'topic';
