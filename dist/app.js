(() => {
  ...
  function render() {
    ...
    $('book-header').innerHTML=`<div class="book-copy"><p class="eyebrow">THE VAHINI SERIES · ENGLISH STUDY</p><h2>${escape(b.title)}</h2><p class="description">${escape(b.description)}</p><div class="book-meta"><span>${b.topics.length} study topics</span><span class="meta-divider" aria-hidden="true"></span><a href="${sourceLink(b.sourceUrl)}" target="_blank" rel="noopener noreferrer">Read original book ↗</a>${(b.pdfUrls || (b.pdfUrl?[{label:'Book',url:b.pdfUrl}]:[])).map(p=>`<a href="${sourceLink(p.url)}" target="_blank" rel="noopener noreferrer">${escape(p.label)} PDF ↗</a>`).join('')}</div></div>${art}`;
    $('topic-position').textContent=`Topic ${topicIndex+1} of ${b.topics.length}`;
    $('topics').innerHTML=b.topics.map((x,i)=>`<button data-topic="${i}" class="topic-button ${i===topicIndex?'active':''}" aria-pressed="${i===topicIndex}"><span>${String(i+1).padStart(2,'0')}</span>${escape(x.title)}</button>`).join('');
  ...
  function referenceCard(t) {
    return `<aside class="source-card"><p class="eyebrow">READ AT THE SOURCE</p><h4>${escape(t.chapter)}</h4><p>Return to the original text for the full teaching and context.</p><a href="${sourceLink(t.sourceUrl)}" target="_blank" rel="noopener noreferrer">Open source text ↗</a></aside>`;
  }
  function renderActivity() {
    const t=currentTopic(), title=`<h3 class="activity-title">${escape(t.title)}</h3>`;
    if(activeTab==='summary') {
      $('activity').innerHTML=`${title}<div class="summary-grid"><div><p class="summary-text">${escape(t.summary)}</p><div class="action-row"><button class="primary-button" data-tab="questions">Explore the questions</button><button class="secondary-button" data-tab="quiz">Try the quiz</button></div><p class="study-note">A brief study summary based on the linked Vahini text.</p></div>${referenceCard(t)}</div>`;
      ...
      $('activity').innerHTML=`${title}<p class="quiz-intro">${t.quiz.length} questions · Choose one answer for each. Your selections stay in place as you move between pages.</p><p class="exercise-note">Practice scenarios are imagined situations based on this topic’s teaching; they are not events or quotations from the book.</p>${attempt.checked?`<div class="quiz-result" role="status"><strong>${score} of ${t.quiz.length} correct</strong>${score===t.quiz.length?'Well done. Take a moment to reflect on what you learned.':'Review the explanations on each page, revisit the text, and try again.'}</div>`:''}<div class="quiz-toolbar"><div><strong>Questions ${start+1}–${end} of ${t.quiz.length}</strong><span id="answer-progress">${answered} of ${t.quiz.length} answered</span></div><nav class="quiz-pages" aria-label="Quiz pages">${pageButtons}</nav></div><progress id="quiz-progress" max="${t.quiz.length}" value="${answered}" aria-label="Questions answered"></progress><form id="quiz-form">${t.quiz.slice(start,end).map((q,offset)=>{
